...
from utility.metadata import CustomMetadata
from utility.models import BaseModel
from utility.pagination import CustomPagination
from utility.renderer import CustomRenderer
...
class BaseView(APIView, CustomPagination):
    ...
    def get_serializer_context(self):
        ...
        if self.request.user:
            user = User.objects.filter(email=self.request.user.id).first()
        return {
    ...
    def get(self, request: Request):
        ...
        if request.query_params.get("csv") and request.query_params.get("csv") == True:
            return self.send_csv(
                request,
                self.get_queryset(request, None, None, True, **query_params),
